...
void DLXInstrInfo::copyPhysReg(MachineBasicBlock &MBB,
  ...
  if (!DLX::GPRRegClass.contains(DestinationRegister, SourceRegister)) {
    llvm_unreachable("Impossible reg-to-reg copy");
  }

  BuildMI(MBB, Position, DL, get(DLX::OR_I_LO), DestinationRegister)
      .addReg(SourceRegister, getKillRegState(KillSource))
      .addImm(0);
}

void DLXInstrInfo::storeRegToStackSlot(
  ...
  if (!DLX::GPRRegClass.hasSubClassEq(RegisterClass)) {
    llvm_unreachable("Can't store this register to stack slot");
  }
  BuildMI(MBB, Position, DL, get(DLX::SW_RI))
...
void DLXInstrInfo::loadRegFromStackSlot(
    MachineBasicBlock &MBB, MachineBasicBlock::iterator Position,
  ...
  BuildMI(MBB, Position, DL, get(DLX::LDW_RI), DestinationRegister)
      .addFrameIndex(FrameIndex)
      .addImm(0)
      .addImm(LPAC::ADD);
}

bool DLXInstrInfo::areMemAccessesTriviallyDisjoint(
...
bool DLXInstrInfo::expandPostRAPseudo(MachineInstr & /*MI*/) const {
  return false;
}
...
bool DLXInstrInfo::analyzeCompare(const MachineInstr &MI, unsigned &SrcReg,
                                    unsigned &SrcReg2, int &CmpMask,
                                    int &CmpValue) const {
  switch (MI.getOpcode()) {
  default:
    break;
  case DLX::SFSUB_F_RI_LO:
  case DLX::SFSUB_F_RI_HI:
...
inline static bool isRedundantFlagInstr(MachineInstr *CmpI, unsigned SrcReg,
                                        ...
                                        MachineInstr *OI) {
  if (CmpI->getOpcode() == DLX::SFSUB_F_RR &&
      OI->getOpcode() == DLX::SUB_R &&
  ...
  if (((CmpI->getOpcode() == DLX::SFSUB_F_RI_LO &&
        OI->getOpcode() == DLX::SUB_I_LO) ||
       (CmpI->getOpcode() == DLX::SFSUB_F_RI_HI &&
        OI->getOpcode() == DLX::SUB_I_HI)) &&
      OI->getOperand(1).getReg() == SrcReg &&
...
inline static unsigned flagSettingOpcodeVariant(unsigned OldOpcode) {
  switch (OldOpcode) {
  case DLX::ADD_I_HI:
    return DLX::ADD_F_I_HI;
  case DLX::ADD_I_LO:
    return DLX::ADD_F_I_LO;
  case DLX::ADD_R:
    return DLX::ADD_F_R;
  case DLX::ADDC_I_HI:
    return DLX::ADDC_F_I_HI;
  case DLX::ADDC_I_LO:
    return DLX::ADDC_F_I_LO;
  case DLX::ADDC_R:
    return DLX::ADDC_F_R;
  case DLX::AND_I_HI:
    return DLX::AND_F_I_HI;
  case DLX::AND_I_LO:
    return DLX::AND_F_I_LO;
  case DLX::AND_R:
    return DLX::AND_F_R;
  case DLX::OR_I_HI:
    return DLX::OR_F_I_HI;
  case DLX::OR_I_LO:
    return DLX::OR_F_I_LO;
  case DLX::OR_R:
    return DLX::OR_F_R;
  case DLX::SL_I:
    return DLX::SL_F_I;
  ...
  case DLX::SRA_R:
    return DLX::SRA_F_R;
  case DLX::SUB_I_HI:
    return DLX::SUB_F_I_HI;
  case DLX::SUB_I_LO:
    return DLX::SUB_F_I_LO;
  case DLX::SUB_R:
    return DLX::SUB_F_R;
  case DLX::SUBB_I_HI:
    return DLX::SUBB_F_I_HI;
  case DLX::SUBB_I_LO:
    return DLX::SUBB_F_I_LO;
  case DLX::SUBB_R:
    return DLX::SUBB_F_R;
  case DLX::XOR_I_HI:
    return DLX::XOR_F_I_HI;
  case DLX::XOR_I_LO:
    return DLX::XOR_F_I_LO;
  case DLX::XOR_R:
    return DLX::XOR_F_R;
  default:
    return DLX::NOP;
...
bool DLXInstrInfo::optimizeCompareInstr(
  ...
  if (SrcReg2 != 0)
    ...
    if (CmpInstr.getOpcode() == DLX::SFSUB_F_RI_LO)
      MI = nullptr;
    else
      return false;
  }
  ...
  for (--I; I != E; --I) {
    ...
    if (Instr.modifiesRegister(DLX::SR, TRI) ||
        Instr.readsRegister(DLX::SR, TRI))
  ...
  if (flagSettingOpcodeVariant(MI->getOpcode()) != DLX::NOP) {
    bool isSafe = false;
    ...
    while (!isSafe && ++I != E) {
      ...
      for (unsigned IO = 0, EO = Instr.getNumOperands(); !isSafe && IO != EO;
        ...
        if (MO.isRegMask() && MO.clobbersPhysReg(DLX::SR)) {
          isSafe = true;
          break;
        }
        if (!MO.isReg() || MO.getReg() != DLX::SR)
          continue;
        if (MO.isDef()) {
          isSafe = true;
          break;
...
bool DLXInstrInfo::analyzeSelect(const MachineInstr &MI,
                                   SmallVectorImpl<MachineOperand> &Cond,
                                   unsigned &TrueOp, unsigned &FalseOp,
                                   bool &Optimizable) const {
  assert(MI.getOpcode() == DLX::SELECT && "unknown select instruction");
  // Select operands:
  // 0: Def.
  // 1: True use.
  // 2: False use.
...
bool DLXInstrInfo::analyzeBranch(MachineBasicBlock &MBB,
  ...
  while (Instruction != MBB.begin()) {
    ...
    if (Instruction->getOpcode() == DLX::BT) {
      if (!AllowModify) {
...
// reversed.
bool DLXInstrInfo::reverseBranchCondition(
    SmallVectorImpl<llvm::MachineOperand> &Condition) const {
  assert((Condition.size() == 1) &&
         "DLX branch conditions should have one component.");
...
unsigned DLXInstrInfo::insertBranch(MachineBasicBlock &MBB,
                                      MachineBasicBlock *TrueBlock,
                                      MachineBasicBlock *FalseBlock,
  ...
  if (Condition.empty()) {
    ...
    BuildMI(&MBB, DL, get(DLX::BT)).addMBB(TrueBlock);
    return 1;
  }
  ...
  BuildMI(&MBB, DL, get(DLX::BRCC)).addMBB(TrueBlock).addImm(ConditionalCode);
  ...
  BuildMI(&MBB, DL, get(DLX::BT)).addMBB(FalseBlock);
  return 2;
}

unsigned DLXInstrInfo::removeBranch(MachineBasicBlock &MBB,
  ...
  while (Instruction != MBB.begin()) {
    ...
    if (Instruction->isDebugInstr())
      continue;
    if (Instruction->getOpcode() != DLX::BT &&
        Instruction->getOpcode() != DLX::BRCC) {
      break;
    }
...
unsigned DLXInstrInfo::isLoadFromStackSlotPostFE(const MachineInstr &MI,
                                                   int &FrameIndex) const {
  if (MI.getOpcode() == DLX::LDW_RI) {
    unsigned Reg;
...
bool DLXInstrInfo::getMemOpBaseRegImmOfsWidth(
...
bool DLXInstrInfo::getMemOpBaseRegImmOfs(
