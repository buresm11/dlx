//===-- DLXAsmParser.cpp - Parse DLX assembly to MCInst instructions --===//
//
...
class DLXAsmParser : public MCTargetAsmParser {
  // Parse operands
  std::unique_ptr<DLXOperand> parseRegister();

  std::unique_ptr<DLXOperand> parseImmediate();

  std::unique_ptr<DLXOperand> parseIdentifier();
...
public:
  DLXAsmParser(const MCSubtargetInfo &STI, MCAsmParser &Parser,
...
// DLXOperand - Instances of this class represented a parsed machine
// instruction
struct DLXOperand : public MCParsedAsmOperand {
  ...
  explicit DLXOperand(KindTy Kind) : MCParsedAsmOperand(), Kind(Kind) {}
...
public:
  ...
  bool isHiImm16() {
    ...
    if (const DLXMCExpr *SymbolRefExpr = dyn_cast<DLXMCExpr>(Imm.Value))
      return SymbolRefExpr->getKind() == DLXMCExpr::VK_DLX_ABS_HI;

    // Binary expression
    if (const MCBinaryExpr *BinaryExpr = dyn_cast<MCBinaryExpr>(Imm.Value))
      if (const DLXMCExpr *SymbolRefExpr =
              dyn_cast<DLXMCExpr>(BinaryExpr->getLHS()))
        return SymbolRefExpr->getKind() == DLXMCExpr::VK_DLX_ABS_HI;
  ...
  bool isLoImm16() {
    ...
    // Symbolic reference expression
    if (const DLXMCExpr *SymbolRefExpr = dyn_cast<DLXMCExpr>(Imm.Value))
      return SymbolRefExpr->getKind() == DLXMCExpr::VK_DLX_ABS_LO;
  ...
  bool isLoImm21() {
    ...
    // Symbolic reference expression
    if (const DLXMCExpr *SymbolRefExpr = dyn_cast<DLXMCExpr>(Imm.Value))
      return SymbolRefExpr->getKind() == DLXMCExpr::VK_DLX_None;
  ...
  void addLoImm16Operands(MCInst &Inst, unsigned N) const {
    ...
    if (const MCConstantExpr *ConstExpr = dyn_cast<MCConstantExpr>(getImm()))
      Inst.addOperand(
          MCOperand::createImm(static_cast<int32_t>(ConstExpr->getValue())));
    else if (isa<DLXMCExpr>(getImm())) {
#ifndef NDEBUG
      const DLXMCExpr *SymbolRefExpr = dyn_cast<DLXMCExpr>(getImm());
      assert(SymbolRefExpr->getKind() == DLXMCExpr::VK_DLX_ABS_LO);
#endif
      Inst.addOperand(MCOperand::createExpr(getImm()));
    } else if (isa<MCBinaryExpr>(getImm())) {
#ifndef NDEBUG
      const MCBinaryExpr *BinaryExpr = dyn_cast<MCBinaryExpr>(getImm());
      assert(isa<DLXMCExpr>(BinaryExpr->getLHS()) &&
             cast<DLXMCExpr>(BinaryExpr->getLHS())->getKind() ==
                 DLXMCExpr::VK_DLX_ABS_LO);
#endif
      Inst.addOperand(MCOperand::createExpr(getImm()));
    } else
...
#ifndef NDEBUG
      const DLXMCExpr *SymbolRefExpr = dyn_cast<DLXMCExpr>(getImm());
      assert(SymbolRefExpr->getKind() == DLXMCExpr::VK_DLX_ABS_HI);
#endif
      Inst.addOperand(MCOperand::createExpr(getImm()));
    } else if (isa<MCBinaryExpr>(getImm())) {
#ifndef NDEBUG
  ...
  void addLoImm21Operands(MCInst &Inst, unsigned N) const {
    ...
    if (const MCConstantExpr *ConstExpr = dyn_cast<MCConstantExpr>(getImm()))
      ...
      assert(SymbolRefExpr &&
             SymbolRefExpr->getKind() == DLXMCExpr::VK_DLX_None);
#endif
      Inst.addOperand(MCOperand::createExpr(getImm()));
  ...
  static std::unique_ptr<DLXOperand> CreateToken(StringRef Str, SMLoc Start) {
    auto Op = make_unique<DLXOperand>(TOKEN);
  ...
  static std::unique_ptr<DLXOperand> createReg(unsigned RegNum, SMLoc Start,
                                                 SMLoc End) {
    auto Op = make_unique<DLXOperand>(REGISTER);
  ...
  static std::unique_ptr<DLXOperand> createImm(const MCExpr *Value,
                                                 SMLoc Start, SMLoc End) {
    auto Op = make_unique<DLXOperand>(IMMEDIATE);
    Op->Imm.Value = Value;
    Op->StartLoc = Start;
    Op->EndLoc = End;
    return Op;
  ...
  static std::unique_ptr<DLXOperand>
  MorphToMemImm(std::unique_ptr<DLXOperand> Op) {
  ...
  static std::unique_ptr<DLXOperand>
  MorphToMemRegReg(unsigned BaseReg, std::unique_ptr<DLXOperand> Op,
                   unsigned AluOp) {
  ...
  static std::unique_ptr<DLXOperand>
  MorphToMemRegImm(unsigned BaseReg, std::unique_ptr<DLXOperand> Op,
                   unsigned AluOp) {
    const MCExpr *Imm = Op->getImm();
...
bool DLXAsmParser::ParseDirective(AsmToken /*DirectiveId*/) { return true; }

bool DLXAsmParser::MatchAndEmitInstruction(SMLoc IdLoc, unsigned &Opcode,
                                             OperandVector &Operands,
                                             MCStreamer &Out,
                                             uint64_t &ErrorInfo,
  ...
  case Match_InvalidOperand: {
    ...
    if (ErrorInfo != ~0U) {
      ...
      ErrorLoc = ((DLXOperand &)*Operands[ErrorInfo]).getStartLoc();
      if (ErrorLoc == SMLoc())
        ErrorLoc = IdLoc;
    }
...
std::unique_ptr<DLXOperand> DLXAsmParser::parseRegister() {
  ...
  if (Lexer.getKind() == AsmToken::Identifier) {
    ...
    return DLXOperand::createReg(RegNum, Start, End);
  }
  return nullptr;
}

bool DLXAsmParser::ParseRegister(unsigned &RegNum, SMLoc &StartLoc,
                                   SMLoc &EndLoc) {
  const AsmToken &Tok = getParser().getTok();
  StartLoc = Tok.getLoc();
  EndLoc = Tok.getEndLoc();
  std::unique_ptr<DLXOperand> Op = parseRegister();
  if (Op != nullptr)
    RegNum = Op->getReg();
  return (Op == nullptr);
}

std::unique_ptr<DLXOperand> DLXAsmParser::parseIdentifier() {
  SMLoc Start = Parser.getTok().getLoc();
  SMLoc End = SMLoc::getFromPointer(Parser.getTok().getLoc().getPointer() - 1);
  const MCExpr *Res, *RHS = nullptr;
  DLXMCExpr::VariantKind Kind = DLXMCExpr::VK_DLX_None;
  ...
  Res = DLXMCExpr::create(Kind, Expr, getContext());

  // Nest if this was an addition
  if (RHS)
    Res = MCBinaryExpr::createAdd(Res, RHS, getContext());

  return DLXOperand::createImm(Res, Start, End);
}

std::unique_ptr<DLXOperand> DLXAsmParser::parseImmediate() {
  SMLoc Start = Parser.getTok().getLoc();
  SMLoc End = SMLoc::getFromPointer(Parser.getTok().getLoc().getPointer() - 1);
  ...
  case AsmToken::Dot:
    if (!Parser.parseExpression(ExprVal))
      return DLXOperand::createImm(ExprVal, Start, End);
    LLVM_FALLTHROUGH;
  default:
    return nullptr;
  }
...
unsigned DLXAsmParser::parseAluOperator(bool PreOp, bool PostOp) {
  StringRef IdString;
  Parser.parseIdentifier(IdString);
  unsigned AluCode = LPAC::stringToDLXAluCode(IdString);
...
bool DLXAsmParser::parsePrePost(StringRef Type, int *OffsetValue) {
...
bool shouldBeSls(const DLXOperand &Op) {
  ...
  if (const DLXMCExpr *SymbolRefExpr = dyn_cast<DLXMCExpr>(Op.getImm()))
    return SymbolRefExpr->getKind() == DLXMCExpr::VK_DLX_None;
  ...
  if (const MCBinaryExpr *BinaryExpr = dyn_cast<MCBinaryExpr>(Op.getImm())) {
    const DLXMCExpr *LHSSymbolRefExpr =
        dyn_cast<DLXMCExpr>(BinaryExpr->getLHS());
    return (LHSSymbolRefExpr &&
            LHSSymbolRefExpr->getKind() == DLXMCExpr::VK_DLX_None);
  }
  return false;
}
...
DLXAsmParser::parseMemoryOperand(OperandVector &Operands) {
  ...
  if (Operands[0]->isToken())
    Type = static_cast<DLXOperand *>(Operands[0].get())->getToken();
  ...
  Parser.Lex(); // Eat the '['.
  std::unique_ptr<DLXOperand> Offset = nullptr;
  if (Op)
    Offset.swap(Op);
  ...
  if (!Op) {
    if (!Offset) {
      if ((Op = parseImmediate()) && Lexer.is(AsmToken::RBrac)) {
        ...
        // SLS, the rest as RM.
        if (shouldBeSls(*Op)) {
          Operands.push_back(DLXOperand::MorphToMemImm(std::move(Op)));
        } else {
          ...
          Operands.push_back(DLXOperand::MorphToMemRegImm(
              DLX::R0, std::move(Op), LPAC::ADD));
        }
        return MatchOperand_Success;
      }
    }
  ...
  if (Lexer.is(AsmToken::RBrac)) {
    ...
    if (!Offset) {
      ...
      Offset = DLXOperand::createImm(OffsetConstExpr, Start, End);
    }
  } else {
  ...
  Operands.push_back(
      Offset->isImm()
          ? DLXOperand::MorphToMemRegImm(BaseReg, std::move(Offset), AluOp)
          : DLXOperand::MorphToMemRegReg(BaseReg, std::move(Offset), AluOp));
...
StringRef DLXAsmParser::splitMnemonic(StringRef Name, SMLoc NameLoc,
                                        OperandVector *Operands) {
  ...
  if (Mnemonic[0] == 'b' ||
    ...
    LPCC::CondCode CondCode =
        LPCC::suffixToDLXCondCode(Mnemonic.substr(1, Next));
    if (CondCode != LPCC::UNKNOWN) {
      Mnemonic = Mnemonic.slice(0, 1);
      Operands->push_back(DLXOperand::CreateToken(Mnemonic, NameLoc));
      Operands->push_back(DLXOperand::createImm(
          MCConstantExpr::create(CondCode, getContext()), NameLoc, NameLoc));
      if (IsBRR) {
        Operands->push_back(DLXOperand::CreateToken(".r", NameLoc));
      }
      return Mnemonic;
    }
  }
  ...
  if (Mnemonic.startswith("sel") ||
    ...
    LPCC::CondCode CondCode = LPCC::suffixToDLXCondCode(Mnemonic);
    if (CondCode != LPCC::UNKNOWN) {
...
static bool IsMemoryAssignmentError(const OperandVector &Operands) {
  ...
  if (DLXOperand *PossibleAluOp =
          static_cast<DLXOperand *>(Operands[PossibleAluOpIdx].get()))
...
static bool IsRegister(const MCParsedAsmOperand &op) {
  return static_cast<const DLXOperand &>(op).isReg();
}
...
static bool MaybePredicatedInst(const OperandVector &Operands) {
  ...
  return StringSwitch<bool>(
             static_cast<const DLXOperand &>(*Operands[0]).getToken())
...
bool DLXAsmParser::ParseInstruction(ParseInstructionInfo & /*Info*/,
  ...
  if (Lexer.is(AsmToken::EndOfStatement) && Name == "st" &&
    ...
    Operands.insert(Operands.begin(), DLXOperand::CreateToken("s", NameLoc));
  ...
  if (Lexer.is(AsmToken::EndOfStatement) && Name.startswith("bt") &&
    ...
    Operands.insert(Operands.begin(), DLXOperand::CreateToken("bt", NameLoc));
  }
...
extern "C" void LLVMInitializeDLXAsmParser() {
