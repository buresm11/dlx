...
static DLX::Fixups FixupKind(const MCExpr *Expr) {
  if (isa<MCSymbolRefExpr>(Expr))
    return DLX::FIXUP_DLX_21;
  if (const DLXMCExpr *McExpr = dyn_cast<DLXMCExpr>(Expr)) {
    DLXMCExpr::VariantKind ExprKind = McExpr->getKind();
    switch (ExprKind) {
    case DLXMCExpr::VK_DLX_None:
      return DLX::FIXUP_DLX_21;
    case DLXMCExpr::VK_DLX_ABS_HI:
      return DLX::FIXUP_DLX_HI16;
    case DLXMCExpr::VK_DLX_ABS_LO:
      return DLX::FIXUP_DLX_LO16;
    }
  }
  return DLX::Fixups(0);
}
...
// getMachineOpValue - Return binary encoding of operand. If the machine
// operand requires relocation, record the relocation and return zero.
unsigned DLXMCCodeEmitter::getMachineOpValue(
    const MCInst &Inst, const MCOperand &MCOp, SmallVectorImpl<MCFixup> &Fixups,
    const MCSubtargetInfo &SubtargetInfo) const {
  if (MCOp.isReg())
    return getDLXRegisterNumbering(MCOp.getReg());
  if (MCOp.isImm())
  ...
  assert(isa<DLXMCExpr>(Expr) || Expr->getKind() == MCExpr::SymbolRef);
...
static unsigned adjustPqBits(const MCInst &Inst, unsigned Value,
  ...
  if (!LPAC::isPostOp(AluCode) &&
      ((Op2.isImm() && Op2.getImm() != 0) ||
       (Op2.isReg() && Op2.getReg() != DLX::R0) || (Op2.isExpr())))
    Value |= (1 << PBitShift);
...
unsigned
DLXMCCodeEmitter::adjustPqBitsRmAndRrm(const MCInst &Inst, unsigned Value,
                                         const MCSubtargetInfo &STI) const {
  return adjustPqBits(Inst, Value, 17, 16);
}

unsigned
DLXMCCodeEmitter::adjustPqBitsSpls(const MCInst &Inst, unsigned Value,
                                     const MCSubtargetInfo &STI) const {
  return adjustPqBits(Inst, Value, 11, 10);
}

void DLXMCCodeEmitter::encodeInstruction(
...
// Encode DLX Memory Operand
unsigned DLXMCCodeEmitter::getRiMemoryOpValue(
  ...
  Encoding = (getDLXRegisterNumbering(Op1.getReg()) << 18);
  if (Op2.isImm()) {
...
unsigned DLXMCCodeEmitter::getRrMemoryOpValue(
    const MCInst &Inst, unsigned OpNo, SmallVectorImpl<MCFixup> &Fixups,
    const MCSubtargetInfo &SubtargetInfo) const {
  unsigned Encoding;
  const MCOperand Op1 = Inst.getOperand(OpNo + 0);
  ...
  assert(Op1.isReg() && "First operand is not register.");
  Encoding = (getDLXRegisterNumbering(Op1.getReg()) << 15);
  assert(Op2.isReg() && "Second operand is not register.");
  Encoding |= (getDLXRegisterNumbering(Op2.getReg()) << 10);

  assert(AluMCOp.isImm() && "Third operator is not immediate.");
  // Set BBB
  unsigned AluOp = AluMCOp.getImm();
  Encoding |= LPAC::encodeDLXAluCode(AluOp) << 5;
  // Set P and Q
...
unsigned
DLXMCCodeEmitter::getSplsOpValue(const MCInst &Inst, unsigned OpNo,
                                   SmallVectorImpl<MCFixup> &Fixups,
                                   const MCSubtargetInfo &SubtargetInfo) const {
  unsigned Encoding;
  const MCOperand Op1 = Inst.getOperand(OpNo + 0);
  ...
  Encoding = (getDLXRegisterNumbering(Op1.getReg()) << 12);
  if (Op2.isImm()) {
...
unsigned DLXMCCodeEmitter::getBranchTargetOpValue(
    const MCInst &Inst, unsigned OpNo, SmallVectorImpl<MCFixup> &Fixups,
    const MCSubtargetInfo &SubtargetInfo) const {
  ...
  Fixups.push_back(MCFixup::create(
      0, MCOp.getExpr(), static_cast<MCFixupKind>(DLX::FIXUP_DLX_25)));
...
llvm::MCCodeEmitter *
llvm::createDLXMCCodeEmitter(const MCInstrInfo &InstrInfo,
                               const MCRegisterInfo & /*MRI*/,
                               MCContext &context) {
  return new DLXMCCodeEmitter(InstrInfo, context);
}
