//===-- DLXTargetTransformInfo.h - DLX specific TTI ---------*- C++ -*-===//
//
...
// DLX target machine. It uses the target's detailed information to
// provide more precise answers to certain TTI queries, while letting the
// target independent and default TTI implementations handle the rest.
//
//===----------------------------------------------------------------------===//

#ifndef LLVM_LIB_TARGET_DLX_DLXTARGETTRANSFORMINFO_H
#define LLVM_LIB_TARGET_DLX_DLXTARGETTRANSFORMINFO_H
...
namespace llvm {
class DLXTTIImpl : public BasicTTIImplBase<DLXTTIImpl> {
  typedef BasicTTIImplBase<DLXTTIImpl> BaseT;
  typedef TargetTransformInfo TTI;
  friend BaseT;

  const DLXSubtarget *ST;
  const DLXTargetLowering *TLI;

  const DLXSubtarget *getST() const { return ST; }
  const DLXTargetLowering *getTLI() const { return TLI; }

public:
  explicit DLXTTIImpl(const DLXTargetMachine *TM, const Function &F)
...
#endif // LLVM_LIB_TARGET_DLX_DLXTARGETTRANSFORMINFO_H
