...
static cl::opt<int> DLXLowerConstantMulThreshold(
    "DLX-constant-mul-threshold", cl::Hidden,
...
DLXTargetLowering::DLXTargetLowering(const TargetMachine &TM,
  ...
  setStackPointerRegisterToSaveRestore(DLX::SP);
...
//===----------------------------------------------------------------------===//
//                       DLX Inline Assembly Support
//===----------------------------------------------------------------------===//

unsigned DLXTargetLowering::getRegisterByName(const char *RegName, EVT /*VT*/,
                                                SelectionDAG & /*DAG*/) const {
...
DLXTargetLowering::getRegForInlineAsmConstraint(const TargetRegisterInfo *TRI,
  ...
  if (Constraint.size() == 1)
    ...
    switch (Constraint[0]) {
    case 'r': // GENERAL_REGS
      return std::make_pair(0U, &DLX::GPRRegClass);
    default:
      break;
    }
...
DLXTargetLowering::getSingleConstraintMatchWeight(
...
void DLXTargetLowering::LowerAsmOperandForConstraint(
...
static bool CC_DLX32_VarArg(unsigned ValNo, MVT ValVT, MVT LocVT,
  ...
  if (ValNo < NumFixedArgs) {
    return CC_DLX32(ValNo, ValVT, LocVT, LocInfo, ArgFlags, State);
  }
...
SDValue DLXTargetLowering::LowerCall(TargetLowering::CallLoweringInfo &CLI,
  ...
  // DLX target does not yet support tail call optimization.
  IsTailCall = false;
...
SDValue DLXTargetLowering::LowerCCCArguments(
    SDValue Chain, CallingConv::ID CallConv, bool IsVarArg,
    const SmallVectorImpl<ISD::InputArg> &Ins, const SDLoc &DL,
    SelectionDAG &DAG, SmallVectorImpl<SDValue> &InVals) const {
  ...
  DLXMachineFunctionInfo *DLXMFI = MF.getInfo<DLXMachineFunctionInfo>();
  ...
  if (CallConv == CallingConv::Fast) {
    CCInfo.AnalyzeFormalArguments(Ins, CC_DLX32_Fast);
  } else {
    CCInfo.AnalyzeFormalArguments(Ins, CC_DLX32);
  }
  ...
  for (unsigned i = 0, e = ArgLocs.size(); i != e; ++i) {
    ...
    if (VA.isRegLoc()) {
      ...
      case MVT::i32: {
        unsigned VReg = RegInfo.createVirtualRegister(&DLX::GPRRegClass);
  ...
  // The DLX ABI for returning structs by value requires that we copy
  // the sret argument into rv for the return. Save the argument into
  // a virtual register so that we can access it from the return points.
  if (MF.getFunction().hasStructRetAttr()) {
    unsigned Reg = DLXMFI->getSRetReturnReg();
    if (!Reg) {
      Reg = MF.getRegInfo().createVirtualRegister(getRegClassFor(MVT::i32));
      DLXMFI->setSRetReturnReg(Reg);
    }
  ...
  if (IsVarArg) {
    ...
    DLXMFI->setVarArgsFrameIndex(FI);
  }
...
SDValue
DLXTargetLowering::LowerReturn(SDValue Chain, CallingConv::ID CallConv,
                                 bool IsVarArg,
  ...
  CCInfo.AnalyzeReturn(Outs, RetCC_DLX32);
  ...
  if (DAG.getMachineFunction().getFunction().hasStructRetAttr()) {
    ...
    Chain = DAG.getCopyToReg(Chain, DL, DLX::RV, Val, Flag);
    Flag = Chain.getValue(1);
    RetOps.push_back(
        DAG.getRegister(DLX::RV, getPointerTy(DAG.getDataLayout())));
  }

  RetOps[0] = Chain; // Update chain

  unsigned Opc = DLXISD::RET_FLAG;
  if (Flag.getNode())
...
SDValue DLXTargetLowering::LowerCCCCallTo(
  ...
  if (NumFixedArgs)
    CCInfo.AnalyzeCallOperands(Outs, CC_DLX32_VarArg);
  else {
    if (CallConv == CallingConv::Fast)
      CCInfo.AnalyzeCallOperands(Outs, CC_DLX32_Fast);
    else
      CCInfo.AnalyzeCallOperands(Outs, CC_DLX32);
  }
  ...
  for (unsigned I = 0, J = 0, E = ArgLocs.size(); I != E; ++I) {
    ...
    if (VA.isRegLoc()) {
      ...
      if (StackPtr.getNode() == nullptr)
        StackPtr = DAG.getCopyFromReg(Chain, DL, DLX::SP,
  ...
  uint8_t OpFlag = DLXII::MO_NO_FLAG;
  if (G) {
  ...
  Chain = DAG.getNode(DLXISD::CALL, DL, NodeTys,
...
SDValue DLXTargetLowering::LowerCallResult(
  ...
  CCInfo.AnalyzeCallResult(Ins, RetCC_DLX32);
...
static LPCC::CondCode IntCondCCodeToICC(SDValue CC, const SDLoc &DL,
  ...
  // and DLX only supports integer comparisons, so only provide definitions
  // for them.
  switch (SetCCOpcode) {
  case ISD::SETEQ:
    return LPCC::ICC_EQ;
...
SDValue DLXTargetLowering::LowerBR_CC(SDValue Op, SelectionDAG &DAG) const {
  ...
  SDValue Flag =
      DAG.getNode(DLXISD::SET_FLAG, DL, MVT::Glue, LHS, RHS, TargetCC);

  return DAG.getNode(DLXISD::BR_CC, DL, Op.getValueType(), Chain, Dest,
                     TargetCC, Flag);
}

SDValue DLXTargetLowering::LowerMUL(SDValue Op, SelectionDAG &DAG) const {
  ...
  if (InstrRequired > DLXLowerConstantMulThreshold)
    return SDValue();
...
SDValue DLXTargetLowering::LowerSETCC(SDValue Op, SelectionDAG &DAG) const {
  ...
  return DAG.getNode(DLXISD::SETCC, DL, Op.getValueType(), TargetCC, Flag);
}

SDValue DLXTargetLowering::LowerSELECT_CC(SDValue Op,
                                            SelectionDAG &DAG) const {
  SDValue LHS = Op.getOperand(0);
  SDValue RHS = Op.getOperand(1);
  ...
  return DAG.getNode(DLXISD::SELECT_CC, DL, VTs, TrueV, FalseV, TargetCC,
                     Flag);
}

SDValue DLXTargetLowering::LowerVASTART(SDValue Op, SelectionDAG &DAG) const {
  MachineFunction &MF = DAG.getMachineFunction();
  DLXMachineFunctionInfo *FuncInfo = MF.getInfo<DLXMachineFunctionInfo>();
...
SDValue DLXTargetLowering::LowerDYNAMIC_STACKALLOC(SDValue Op,
                                                     SelectionDAG &DAG) const {
  SDValue Chain = Op.getOperand(0);
  ...
  // For DLX, the outgoing memory arguments area should be on top of the
  ...
  SDValue ArgAdjust = DAG.getNode(DLXISD::ADJDYNALLOC, DL, MVT::i32, Sub);
...
SDValue DLXTargetLowering::LowerRETURNADDR(SDValue Op,
                                             SelectionDAG &DAG) const {
  MachineFunction &MF = DAG.getMachineFunction();
  MachineFrameInfo &MFI = MF.getFrameInfo();
...
SDValue DLXTargetLowering::LowerFRAMEADDR(SDValue Op,
                                            SelectionDAG &DAG) const {
  MachineFrameInfo &MFI = DAG.getMachineFunction().getFrameInfo();
  ...
  EVT VT = Op.getValueType();
  SDLoc DL(Op);
  SDValue FrameAddr = DAG.getCopyFromReg(DAG.getEntryNode(), DL, DLX::FP, VT);
  unsigned Depth = cast<ConstantSDNode>(Op.getOperand(0))->getZExtValue();
  while (Depth--) {
...
const char *DLXTargetLowering::getTargetNodeName(unsigned Opcode) const {
  switch (Opcode) {
  case DLXISD::ADJDYNALLOC:
    return "DLXISD::ADJDYNALLOC";
  case DLXISD::RET_FLAG:
    return "DLXISD::RET_FLAG";
  case DLXISD::CALL:
    return "DLXISD::CALL";
  case DLXISD::SELECT_CC:
    return "DLXISD::SELECT_CC";
  case DLXISD::SETCC:
    return "DLXISD::SETCC";
  case DLXISD::SUBBF:
    return "DLXISD::SUBBF";
  case DLXISD::SET_FLAG:
    return "DLXISD::SET_FLAG";
  case DLXISD::BR_CC:
    return "DLXISD::BR_CC";
  case DLXISD::Wrapper:
    return "DLXISD::Wrapper";
  case DLXISD::HI:
    return "DLXISD::HI";
  case DLXISD::LO:
    return "DLXISD::LO";
...
SDValue DLXTargetLowering::LowerConstantPool(SDValue Op,
                                               SelectionDAG &DAG) const {
  SDLoc DL(Op);
  ConstantPoolSDNode *N = cast<ConstantPoolSDNode>(Op);
  const Constant *C = N->getConstVal();
  const DLXTargetObjectFile *TLOF =
      static_cast<const DLXTargetObjectFile *>(
  ...
  if (getTargetMachine().getCodeModel() == CodeModel::Small ||
    ...
    SDValue Small = DAG.getTargetConstantPool(
        C, MVT::i32, N->getAlignment(), N->getOffset(), DLXII::MO_NO_FLAG);
    return DAG.getNode(ISD::OR, DL, MVT::i32,
                       DAG.getRegister(DLX::R0, MVT::i32),
                       DAG.getNode(DLXISD::SMALL, DL, MVT::i32, Small));
  } else {
    uint8_t OpFlagHi = DLXII::MO_ABS_HI;
    uint8_t OpFlagLo = DLXII::MO_ABS_LO;
    ...
    Hi = DAG.getNode(DLXISD::HI, DL, MVT::i32, Hi);
    Lo = DAG.getNode(DLXISD::LO, DL, MVT::i32, Lo);
    SDValue Result = DAG.getNode(ISD::OR, DL, MVT::i32, Hi, Lo);
    return Result;
  }
}

SDValue DLXTargetLowering::LowerGlobalAddress(SDValue Op,
                                                SelectionDAG &DAG) const {
  SDLoc DL(Op);
  ...
  if (TLOF->isGlobalInSmallSection(GO, getTargetMachine())) {
    SDValue Small = DAG.getTargetGlobalAddress(
        GV, DL, getPointerTy(DAG.getDataLayout()), Offset, DLXII::MO_NO_FLAG);
    return DAG.getNode(ISD::OR, DL, MVT::i32,
                       DAG.getRegister(DLX::R0, MVT::i32),
                       DAG.getNode(DLXISD::SMALL, DL, MVT::i32, Small));
  } else {
...
SDValue DLXTargetLowering::LowerBlockAddress(SDValue Op,
                                               SelectionDAG &DAG) const {
  SDLoc DL(Op);
...
SDValue DLXTargetLowering::LowerJumpTable(SDValue Op,
                                            SelectionDAG &DAG) const {
  SDLoc DL(Op);
  ...
  if (getTargetMachine().getCodeModel() == CodeModel::Small) {
    SDValue Small = DAG.getTargetJumpTable(
        JT->getIndex(), getPointerTy(DAG.getDataLayout()), DLXII::MO_NO_FLAG);
    return DAG.getNode(ISD::OR, DL, MVT::i32,
                       DAG.getRegister(DLX::R0, MVT::i32),
                       DAG.getNode(DLXISD::SMALL, DL, MVT::i32, Small));
  } else {
...
SDValue DLXTargetLowering::LowerSHL_PARTS(SDValue Op,
                                            SelectionDAG &DAG) const {
  EVT VT = Op.getValueType();
  ...
  // DLX shifts of larger than register sizes are wrapped rather than
...
SDValue DLXTargetLowering::LowerSRL_PARTS(SDValue Op,
                                            SelectionDAG &DAG) const {
  ...
  // Note: This takes advantage of DLX's shift behavior to avoid needing to
...
SDValue DLXTargetLowering::PerformDAGCombine(SDNode *N,
...
void DLXTargetLowering::computeKnownBitsForTargetNode(
