...
//===----------------------------------------------------------------------===//
//
// This file contains the DLX implementation of TargetFrameLowering class.
//
//===----------------------------------------------------------------------===//
...
void DLXFrameLowering::determineFrameLayout(MachineFunction &MF) const {
  MachineFrameInfo &MFI = MF.getFrameInfo();
  const DLXRegisterInfo *LRI = STI.getRegisterInfo();
...
void DLXFrameLowering::replaceAdjDynAllocPseudo(MachineFunction &MF) const {
  ...
  for (MachineFunction::iterator MBB = MF.begin(), E = MF.end(); MBB != E;
    ...
    while (MBBI != MBB->end()) {
      ...
      if (MI.getOpcode() == DLX::ADJDYNALLOC) {
        DebugLoc DL = MI.getDebugLoc();
        unsigned Dst = MI.getOperand(0).getReg();
        unsigned Src = MI.getOperand(1).getReg();

        BuildMI(*MBB, MI, DL, LII.get(DLX::ADD_I_LO), Dst)
            .addReg(Src)
...
void DLXFrameLowering::emitPrologue(MachineFunction &MF,
  ...
  // Push old FP
  // st %fp,-4[*%sp]
  BuildMI(MBB, MBBI, DL, LII.get(DLX::SW_RI))
      .addReg(DLX::FP)
      .addReg(DLX::SP)
      .addImm(-4)
  ...
  // Generate new FP
  // add %sp,8,%fp
  BuildMI(MBB, MBBI, DL, LII.get(DLX::ADD_I_LO), DLX::FP)
      .addReg(DLX::SP)
      .addImm(8)
      .setMIFlag(MachineInstr::FrameSetup);
  ...
  // sub %sp,StackSize,%sp
  if (StackSize != 0) {
    BuildMI(MBB, MBBI, DL, LII.get(DLX::SUB_I_LO), DLX::SP)
        .addReg(DLX::SP)
        .addImm(StackSize)
        .setMIFlag(MachineInstr::FrameSetup);
  }
...
void DLXFrameLowering::emitEpilogue(MachineFunction & /*MF*/,
  ...
  BuildMI(MBB, MBBI, DL, LII.get(DLX::ADD_I_LO), DLX::SP)
      .addReg(DLX::FP)
      .addImm(0);
...
void DLXFrameLowering::determineCalleeSaves(MachineFunction &MF,
  ...
  MachineFrameInfo &MFI = MF.getFrameInfo();
  const DLXRegisterInfo *LRI =
      static_cast<const DLXRegisterInfo *>(STI.getRegisterInfo());
  int Offset = -4;
