//===-- DLXDelaySlotFiller.cpp - DLX delay slot filler ----------------===//
//
...
static cl::opt<bool>
    NopDelaySlotFiller("DLX-nop-delay-filler", cl::init(false),
                       cl::desc("Fill DLX delay slots with NOPs."),
                       cl::Hidden);
...
struct Filler : public MachineFunctionPass {
  ...
  StringRef getPassName() const override { return "DLX Delay Slot Filler"; }

  bool runOnMachineBasicBlock(MachineBasicBlock &MBB);

  bool runOnMachineFunction(MachineFunction &MF) override {
    const DLXSubtarget &Subtarget = MF.getSubtarget<DLXSubtarget>();
...
// createDLXDelaySlotFillerPass - Returns a pass that fills in delay
// slots in DLX MachineFunctions
FunctionPass *
llvm::createDLXDelaySlotFillerPass(const DLXTargetMachine & /*tm*/) {
  return new Filler();
}
  ...
  for (MachineBasicBlock::instr_iterator I = MBB.instr_begin();
    ...
    if (I->getDesc().hasDelaySlot()) {
      ...
      if (I->getOpcode() == DLX::RET) {
        ...
        assert(RI->getOpcode() == DLX::LDW_RI && RI->getOperand(0).isReg() &&
               RI->getOperand(0).getReg() == DLX::FP &&
               RI->getOperand(1).isReg() &&
               RI->getOperand(1).getReg() == DLX::FP &&
               RI->getOperand(2).isImm() && RI->getOperand(2).getImm() == -8);
        ++RI;
        assert(RI->getOpcode() == DLX::ADD_I_LO &&
               RI->getOperand(0).isReg() &&
               RI->getOperand(0).getReg() == DLX::SP &&
               RI->getOperand(1).isReg() &&
               RI->getOperand(1).getReg() == DLX::FP);
        ...
        if (!NopDelaySlotFiller && findDelayInstr(MBB, I, J)) {
          ...
          BuildMI(MBB, std::next(I), DebugLoc(), TII->get(DLX::NOP));
        }
        ++FilledSlots;
      }
